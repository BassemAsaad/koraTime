...
class PendingFriendsAdapter(var usersList: MutableList<FriendRequestModel?>?) :
    RecyclerView.Adapter<PendingFriendsAdapter.ViewHolder>() {
    inner class ViewHolder(val dataBinding: ItemPendingFriendBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val dataBinding: ItemPendingFriendBinding =
            DataBindingUtil.inflate(
                LayoutInflater.from(parent.context), R.layout.item_pending_friend, parent, false
            )
