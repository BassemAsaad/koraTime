...
class CalendarAdapter (private val days: List<Date>?) : RecyclerView.Adapter<CalendarAdapter.CalendarViewHolder>() {
    // Variable to store the currently selected date
    private var selectedDate: Date? = if (days!!.isNotEmpty()) days[0] else null
    inner class CalendarViewHolder(val binding : ItemDateBinding):RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(date:Date ){
            ...
            if (date == selectedDate) {
                ...
                binding.apply {
                    ...
                    calendarDay.setTextColor(ContextCompat.getColor(itemView.context, R.color.dark_grey))
                }
            }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CalendarViewHolder {
        val binding : ItemDateBinding = DataBindingUtil.inflate(LayoutInflater.from(parent.context),
