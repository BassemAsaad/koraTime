...
class FindFriendsAdapter (private var addFriendsAdapter: AddFriendsAdapter) :RecyclerView.Adapter<ViewHolder>(){

    companion object{
        const val VIEW_TYPE_SEARCH = 0
        const val VIEW_TYPE_ADD_FRIENDS = 1
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return when (viewType){
            ...
            VIEW_TYPE_ADD_FRIENDS ->{
                ...
                AddFriendsViewHolder(dataBinding)
            }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            ...
            VIEW_TYPE_ADD_FRIENDS -> {
                (holder as AddFriendsViewHolder).bind(addFriendsAdapter)
            }
        }
    }

    inner class AddFriendsViewHolder(val dataBinding: ItemVerticalRecyclerViewBinding)
        : ViewHolder(dataBinding.root) {
            fun bind(addFriendsAdapter: AddFriendsAdapter) {
                dataBinding.recyclerView.adapter = addFriendsAdapter
            }
        ...
        fun bind(){
            ...
            dataBinding.search.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(newText: String): Boolean {
                    addFriendsAdapter.filterUsers(newText)
                    return true
                }
            })
