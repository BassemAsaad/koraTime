...
class PublicRoomsParentAdapter (private val publicRoomsAdapter: PublicRoomsAdapter) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    companion object{
        const val VIEW_TYPE_SEARCH = 0
        const val VIEW_TYPE_ROOMS = 1
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when(viewType) {
            ...
            VIEW_TYPE_ROOMS -> {
                ...
                PublicRoomsViewHolder(dataBinding)
            }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            ...
            VIEW_TYPE_ROOMS -> {
                (holder as PublicRoomsViewHolder).bind(publicRoomsAdapter)
            }
        }
    }
    inner class PublicRoomsViewHolder (val binding : ItemVerticalRecyclerViewBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(publicRoomsAdapter: PublicRoomsAdapter) {
            binding.recyclerView.adapter = publicRoomsAdapter
        ...
        fun bind(){
            ...
            dataBinding.search.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
                ...
                override fun onQueryTextChange(newText: String): Boolean {
                    publicRoomsAdapter.filterUsers(newText)
                    return true
                }
            })
